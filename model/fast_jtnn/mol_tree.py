...
class MolTreeNode(object):
    """Takes input cliques in smileformat and fills the node requirements (of JT), helps in building JT"""
...
class MolTree(object):
    """Junction Tree construction from tree decomposition"""
    ...
    def __init__(self, smiles):
        ...
        for i, c in enumerate(cliques):
            print i, c
            # get molecule from the clique(of atoms) by refering to original
            # molecule
...
if __name__ == "__main__":
    ...
    for line in sys.stdin:
        smiles = line.split()[0]
        mol = MolTree(smiles)
        for c in mol.nodes:  # from junction tree nodes print it smile strings
            cset.add(c.smiles)
    for x in cset:
        print x
