...
def enum_assemble(node, neighbors, prev_nodes=[], prev_amap=[]):
    ...
    for amap in all_attach_confs:
        ...
        if (
            smiles in cand_smiles
            or check_singleton(cand_mol, node, neighbors) == False
        ):
            continue
        cand_smiles.add(smiles)
...
def check_singleton(cand_mol, ctr_node, nei_nodes):
    rings = [
        node for node in nei_nodes + [ctr_node] if node.mol.GetNumAtoms() > 2
    ]
    singletons = [
...
def check_aroma(cand_mol, ctr_node, nei_nodes):
    rings = [
        node for node in nei_nodes + [ctr_node] if node.mol.GetNumAtoms() >= 3
    ]
    if len(rings) < 2:
    ...
    for atom in cand_mol.GetAtoms():
        if (
            atom.GetAtomMapNum() in benzynes + penzynes
