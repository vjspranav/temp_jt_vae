...
class Sparse_GP:
    ...
    def __init__(
        ...
        self.lls = theano.shared(
            value=np.zeros(input_d).astype(theano.config.floatX),
            name="lls",
            borrow=True,
        )
        self.lsf = theano.shared(
            value=np.zeros(1).astype(theano.config.floatX)[0],
            name="lsf",
            borrow=True,
        )
    ...
    def compute_output(self):
        ...
        v_out = T.exp(self.lsf) + T.dot(
            self.Kxz * T.dot(self.Kxz, self.B), T.ones_like(self.z[:, 0:1])
        )
        ...
        if self.ignore_variances:
            ...
            if exact_output_vars:
                ...
                ll = T.transpose(
                    self.EKxz[:, None, :] * self.EKxz[:, :, None], [1, 2, 0]
                )
                kk = T.transpose(
                    self.Kxz[:, None, :] * self.Kxz[:, :, None], [1, 2, 0]
                )
                v1 = T.transpose(
                    T.sum(
                        T.sum(
                ...
                for j in range(1, n + 1):
                    ...
                    v1 += T.dot(
                        self.kplus * T.dot(self.kplus, self.B2),
                        T.ones_like(self.z[:, 0:1]),
                    )
                    v1 += T.dot(
                        self.kminus * T.dot(self.kminus, self.B2),
                        T.ones_like(self.z[:, 0:1]),
                    )
                    v2 += T.dot(
                        self.kplus * T.dot(self.kplus, self.B),
                        T.ones_like(self.z[:, 0:1]),
                    )
                    v2 += T.dot(
                        self.kminus * T.dot(self.kminus, self.B),
                        T.ones_like(self.z[:, 0:1]),
                    )

                v1 -= T.dot(
                    self.EKxz * T.dot(self.EKxz, self.B2),
                    T.ones_like(self.z[:, 0:1]),
                )
                v2 -= T.dot(
                    self.Kxz * T.dot(self.Kxz, self.B),
                    T.ones_like(self.z[:, 0:1]),
                )
    ...
    def getLogNormalizerCavity(self):
        ...
        return (
            casting(0.5 * self.n_inducing_points * np.log(2 * np.pi))
            + casting(0.5) * T.nlinalg.LogDetPSD()(self.covCavity)
            + casting(0.5)
            * T.dot(
                T.dot(T.transpose(self.meanCavity), self.covCavityInv),
                self.meanCavity,
            )
        )
    ...
    def getLogNormalizerPosterior(self):
        ...
        return (
            ...
            + casting(0.5)
            * T.dot(
                T.dot(T.transpose(self.meanPosterior), self.covPosteriorInv),
                self.meanPosterior,
            )
        )
    ...
    def getContributionToEnergy(self):
        ...
        ) * T.cast(self.input_means.shape[0], "float32") + T.sum(
            self.getLogZ()
        )
    ...
    def initialize(self):
        ...
        M = np.outer(
            np.sum(input_means ** 2, 1), np.ones(input_means.shape[0])
        )
        dist = M - 2 * np.dot(input_means, input_means.T) + M.T
        lls = np.log(
            0.5
            * (
                np.median(dist[np.triu_indices(input_means.shape[0], 1)])
                + 1e-3
            )
        ) * np.ones(input_means.shape[1])
        ...
        L = (
            np.random.normal(
                size=(self.n_inducing_points, self.n_inducing_points)
            )
            * 1.0
        )
    ...
    def getLogZ(self):
        ...
        return (
            -0.5 * T.log(2 * np.pi * self.output_vars)
            - 0.5
            * (self.training_targets - self.output_means) ** 2
            / self.output_vars
        )
    ...
    def compute_log_ei(self, x, incumbent):
        ...
        v_out = T.exp(self.lsf) + T.dot(
            Kxz * T.dot(Kxz, B), T.ones_like(self.z[:, 0:1])
        )  # + T.exp(self.lvar_noise)
        m_out = T.dot(Kxz, KzzInvmeanCavity)
        s = (incumbent - m_out) / T.sqrt(v_out)

        log_ei = T.log(
            (incumbent - m_out) * ratio(s) + T.sqrt(v_out)
        ) + log_n_pdf(s)
    ...
    def compute_log_averaged_ei(self, x, X, randomness, incumbent):
        ...
        KXX = (
            compute_kernel(self.lls, self.lsf, X, X)
            - T.dot(T.dot(KXz, KzzInv), KXz.T)
            + T.eye(X.shape[0]) * self.jitter * T.exp(self.lsf)
        )
        ...
        Kzz_expanded = compute_kernel(
            self.lls, self.lsf, z_expanded, z_expanded
        ) + T.eye(z_expanded.shape[0]) * self.jitter * T.exp(self.lsf)
        ...
        log_ei = T.log(
            (incumbent - new_predictive_mean) * ratio(s)
            + T.sqrt(new_predictive_var)
        ) + log_n_pdf(s)
