...
def compute_kernel(lls, lsf, x, z):
    ...
    r2 = (
        T.outer(T.sum(x * x / lsre, 1), T.ones_like(z[:, 0:1]))
...
def compute_psi1(lls, lsf, xmean, xvar, z):
    ...
    r2_psi1 = (
        T.outer(T.sum(xmean * xmean / lspxvar, 1), T.ones_like(z[:, 0:1]))
        - np.float32(2) * T.dot(xmean / lspxvar, T.transpose(z))
        + T.dot(np.float32(1.0) / lspxvar, T.transpose(z) ** 2)
    )
    psi1 = (
        sf
        * T.outer(constterm2, T.ones_like(z[:, 0:1]))
        * T.exp(-np.float32(0.5) * r2_psi1)
    )
...
def compute_psi1_numpy(lls, lsf, xmean, xvar, z):
    ...
    r2_psi1 = (
        np.outer(np.sum(xmean * xmean / lspxvar, 1), np.ones(z.shape[0]))
        - 2 * np.dot(xmean / lspxvar, z.T)
        + np.dot(1.0 / lspxvar, z.T ** 2)
    )
    psi1 = (
        sf * np.outer(constterm2, np.ones(z.shape[0])) * np.exp(-0.5 * r2_psi1)
    )
    return psi1
...
def compute_psi2(lls, lsf, z, input_means, input_vars):
    ...
    r2b = (
        ...
        - 2
        * T.batched_dot(
            scaled_z_minus_m, np.transpose(scaled_z_minus_m, [0, 2, 1])
        )
    )
    term_2 = T.exp(-r2b)
    ...
    psi2_computed = (
        sf ** casting(2.0) * term_1[:, None, None] * term_2 * term_3
    )
