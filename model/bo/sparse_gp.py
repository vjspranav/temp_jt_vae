...
def global_optimization(
    ...
    (x_optimal, y_opt, opt_info) = spo.fmin_l_bfgs_b(objective,
            X_initial, bounds=lbfgs_bounds, iprint=0, maxiter=150)
    x_optimal = x_optimal.reshape((1, grid.shape[1]))

    return (x_optimal, y_opt)


def adam_theano(loss, all_params, learning_rate=1e-3):
    b1 = 0.9
    b2 = 0.999
    e = 1e-8
    gamma = 1 - 1e-8
    ...
    alpha = learning_rate
    t = theano.shared(casting(1.))
    for (theta_previous, g) in zip(all_params, all_grads):
        m_previous = \
            theano.shared(np.zeros(theta_previous.get_value().shape,
...
class SparseGP:
    ...
    def __init__(
        ...
        self.input_means = \
            theano.shared(value=input_means.astype(theano.config.floatX),
                          borrow=True, name='X')
        self.input_vars = \
            theano.shared(value=input_vars.astype(theano.config.floatX),
                          borrow=True, name='X')
        self.original_training_targets = \
            theano.shared(value=training_targets.astype(theano.config.floatX),
                          borrow=True, name='y')
    ...
    def predict(self, means_test, vars_test):
        ...
        if self.predict_function is None:
            ...
            self.predict_function = theano.function([X, Z],
                    predictions, givens={self.input_means: X,
                    self.input_vars: Z})
    ...
    def train_via_LBFGS(
        ...
        def objective(params):
            ...
            energy_value = energy(input_means, input_vars,
                                  training_targets)
        ...
        (x_opt, y_opt, opt_info) = spo.fmin_l_bfgs_b(objective,
                initial_params, bounds=None, iprint=1,
    ...
    def train_via_ADAM(
        ...
        selected_points = np.random.choice(n_data_points,
                n_data_points, replace=False)[0:min(n_data_points,
                minibatch_size)]
        ...
        process_minibatch_adam = theano.function([X, Z, y], -e,
        ...
        print('Training')
        sys.stdout.flush()
        n_batches = int(np.ceil(1. * n_data_points / minibatch_size))
        for j in range(max_iterations):
            ...
            for i in range(n_batches):
                minibatch_data_means = input_means[i * minibatch_size:
                        min((i + 1) * minibatch_size, n_data_points), :]
                minibatch_data_vars = input_vars[i * minibatch_size:
                        min((i + 1) * minibatch_size, n_data_points), :]
                minibatch_targets = training_targets[i * minibatch_size:
                        min((i + 1) * minibatch_size, n_data_points), :]

                start = time.time()
                current_energy = \
                    process_minibatch_adam(minibatch_data_means,
                        minibatch_data_vars, minibatch_targets)
            ...
            (pred, uncert) = self.predict(input_means_test,
                    input_vars_test)
            test_error = np.sqrt(np.mean((pred - test_targets) ** 2))
            test_ll = np.mean(sps.norm.logpdf(pred - test_targets,
                              scale=np.sqrt(uncert)))

            print('Test error: {} Test ll: {}'.format(test_error,
                  test_ll))
            sys.stdout.flush()
            ...
            for i in range(n_batches):
                ...
                minibatch_data_vars = input_vars[i * minibatch_size:
                        min((i + 1) * minibatch_size, n_data_points), :]
                (pred_new, uncert_new) = \
                    self.predict(minibatch_data_means,
                                 minibatch_data_vars)
                pred = np.concatenate((pred, pred_new), 0)
                uncert = np.concatenate((uncert, uncert_new), 0)

            training_error = np.sqrt(np.mean((pred - training_targets)
                    ** 2))
            training_ll = np.mean(sps.norm.logpdf(pred
                                  - training_targets,
                                  scale=np.sqrt(uncert)))
    ...
    def get_incumbent(
        ...
        function_scalar = theano.function([X], m[0, 0],
                givens={self.input_means: X, self.input_vars: 0 * X})
        function_scalar_gradient = theano.function([X], T.grad(m[0, 0],
                self.input_means), givens={self.input_means: X,
                self.input_vars: 0 * X})
    ...
    def batched_greedy_ei(
        ...
        randomness_numpy = casting(0
                                   * np.random.randn(X_numpy.shape[0],
                                   n_samples).astype(theano.config.floatX))

        randomness = \
            theano.shared(value=randomness_numpy.astype(theano.config.floatX),
                          name='randomness', borrow=True)
        X = theano.shared(value=X_numpy.astype(theano.config.floatX),
                          name='X', borrow=True)
        x = T.matrix('x', dtype=theano.config.floatX)
        log_ei = self.sparse_gp.compute_log_averaged_ei(x, X,
                randomness, incumbent)
