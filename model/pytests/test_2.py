...
@pytest.mark.parametrize("file", ["../molvae/moses-h450L56d3beta0.5/model.iter-2", 
...
@pytest.mark.parametrize("file", ["../bo/print_result.py",
                                  "../bo/run_bo.py",
                                  "../bo/results1/scores0.dat"])
def test_bo_files_exists(file):
    assert os.path.isfile(file)

@pytest.mark.parametrize("file", ["../molopt/optimize.py",
                                    "../molopt/joint-h420-L56-d3-beta0.005/model.iter-4",
                                    "../molopt/vaetrain.py"])
def test_molopt_files_exists(file):
    assert os.path.isfile(file)

@pytest.mark.parametrize("file", ["../data/zinc/vocab.txt",
